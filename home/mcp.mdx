...
description: "Mallary MCP is a remote social media MCP server for AI agents and assistants. Use it to create posts, upload media, get job status, list posts, list comments, post supplied replies, fetch analytics, manage profiles and settings, list connected platforms, manage webhooks, and disconnect platforms through Model Context Protocol."
---
...
If you want an AI agent to post to social media, upload media, reply to comments, check analytics, manage profiles and settings, list connected platforms, manage webhooks, or inspect publishing jobs and platform post URLs without building a custom REST client first, Mallary MCP gives you a structured tool layer over the Mallary platform.
...
Claude Code asks for your Mallary API key when you enable the plugin.
...
## Add to Codex
```bash
codex mcp add mallary --url https://mallary.ai/mcp
```
...
Use `mallary_list_profiles` to find random public profile IDs. Tools that publish, list posts, read comments, read analytics, list platforms, read settings, update settings, or disconnect platforms accept `profile_id` for non-default connection profiles. Omit it to use the default profile.
...
Comment listing and comment replies are available on all Mallary plans. Most other MCP tools require a paid plan.
