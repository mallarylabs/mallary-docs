...
description: "Mallary API is a social media API for developers. Use it to publish, reply, read post analytics and audience counts, list connected platforms, manage webhooks, and disconnect platforms across 10 social sites."
---
...
AI Auto Replies can answer useful questions on posts you publish through Mallary. Mallary uses the business details saved for that profile and the post itself to write a short reply. It skips comments that are not questions or are not about your business.

AI Auto Replies work with Facebook, Instagram, Threads, LinkedIn, X, YouTube, and Bluesky. They are available on Pro and Business plans.

### How it works

1. Mallary checks for new comments on supported posts.
2. It looks for questions about your business.
3. It uses your saved business details and the post to write a reply.
4. It posts the reply from your connected social account.
5. It skips comments that it should not answer.

### How to enable it

First, save these details for the Mallary profile you want to use:
...
Then send `POST /api/v1/settings` with `auto_reply_enabled` set to `true`. Add `profile_id` when you want to update a profile that is not your default profile.

```bash
curl --request POST \
  --url https://mallary.ai/api/v1/settings \
  ...
  --data '{
    ...
    "business_description": "We make simple tools for small teams.",
    "services": "Project planning and team support",
    "contact_info": "Email help@example.com",
    "auto_reply_enabled": true
  }'
```

Leave out `profile_id` to use your default Mallary profile.

After you turn it on, new posts use AI Auto Replies by default. You can set `auto_reply_enabled` to `true` or `false` in one `POST /api/v1/post` request when you want that post to use a different setting.

**Important:** Turning this on lets Mallary publish future replies. Only enable it after the social account owner says yes.
...
Examples include Threads post type selection, TikTok direct-vs-upload mode, Pinterest board selection, and YouTube visibility, video tags, or AI-content disclosure.
...
- Facebook supports `feed`, `story`, and `reel` post types. Reels use one 9:16 MP4 or MOV video that is 3 to 90 seconds long.
- Instagram supports `feed`, `story`, `reel`, and `carousel` through `platform_options.instagram.post_type`. Stories use one image or video, Reels use one video, and carousels use 2 to 10 mixed items. For Reels, use `shareToFeed` to choose profile feed placement. Add `trialParams.graduationStrategy` to publish a Trial Reel. For Facebook Login connections, use `isPaidPartnership` and `brandedContentSponsors` on feed posts, Reels, and carousels.
...
For the full matrix, including LinkedIn, Reddit, Bluesky, and current runtime notes, see:
