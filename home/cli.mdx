...
description: "Mallary CLI is a social media command line tool for developers, operators, CI pipelines, and AI agents. Use it to upload media, create and schedule posts, inspect jobs, fetch analytics, manage webhooks, update settings, list connected platforms, and disconnect platforms."
---
...
If you want a social media CLI for uploading media, creating posts, scheduling content, inspecting jobs, reading analytics, listing connected platforms, managing webhooks, or updating brand settings, Mallary CLI gives you a direct command-line client for the public Mallary platform.
...
- inspect publishing jobs, including platform post IDs and URLs when available
- list grouped posts
- fetch analytics
...
- TikTok defaults to `MEDIA_UPLOAD`, which sends uploads to the creator inbox for final review inside TikTok. Use `platform_options.tiktok.post_mode = DIRECT_POST` only when you want Mallary to publish directly.
...
- use `--profile-id` for non-default profiles when posting, listing posts, reading analytics/settings, listing platforms, or disconnecting a platform
