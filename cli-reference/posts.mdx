...
- local `media[].thumbnail_url` file paths are uploaded to the Mallary CDN first
...
In file mode, put `thumbnail_url` on the media item. Local thumbnail paths are uploaded automatically, and existing remote thumbnail URLs must already be hosted on `https://files.mallary.ai/...`.
...
- TikTok video posts: arbitrary thumbnail images are not sent to TikTok; supplying `thumbnail_url` overrides Mallary's `video_cover_timestamp_ms` behavior and lets TikTok use its default cover.
...
- When `platform_options.threads.post_type` is omitted, Mallary chooses from the media payload:
  no media = `text`,
  one image = `image`,
  one video = `video`,
...
- TikTok and Snapchat do not currently support `comments_under_post`
