...
- `--post-type` works in flag mode when every selected destination should use the same supported type
- use `platform_options` in file mode when destinations need different post types
...
To add a YouTube video to an existing playlist, set `platform_options.youtube.playlist_id` in file mode. The playlist must belong to the connected channel.

To add YouTube video tags, set `platform_options.youtube.tags` to a list of text values in file mode. Each tag can use up to 100 characters. The full list can use up to 500 characters after YouTube counts separators.

To disclose realistic AI-generated or altered content, set `platform_options.youtube.containsSyntheticMedia` to `true` in file mode. YouTube may show a disclosure label on the video.

Current thumbnail support:

- YouTube regular videos: use a `jpg`, `jpeg`, or `png` thumbnail up to 50 MB. Use 16:9.
- YouTube Shorts: YouTube may store the image but show a video frame instead. Mallary returns a warning because the YouTube API cannot confirm the cover viewers will see.
- Facebook feed videos: supported with `jpg`, `jpeg`, or `png` up to 10 MB.
...
- Facebook supports `feed`, `story`, and `reel` through `platform_options.facebook.post_type`. Reels use one 9:16 MP4 or MOV video that is 3 to 90 seconds long.
- Instagram supports `feed`, `story`, `reel`, and `carousel` through `platform_options.instagram.post_type`. Stories use one image or video, Reels use one video, and carousels use 2 to 10 mixed items. For a Reel, use `shareToFeed` for profile feed placement. Add `trialParams.graduationStrategy` in file mode to publish a Trial Reel. With a Facebook Login connection, file mode also supports `isPaidPartnership` and up to two `brandedContentSponsors` on feed posts, Reels, and carousels.
