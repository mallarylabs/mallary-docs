---
title: "Authentication"
description: "Connect to Mallary MCP with OAuth or a Mallary API key."
---

Mallary MCP supports OAuth 2.1 for apps and agents. Mallary API keys remain supported for scripts and existing integrations.

The Mallary plugin for ChatGPT and Codex always uses OAuth. It never asks the user to create, paste, or reveal a Mallary API key.

<Card
  title="Before using the Mallary MCP Server:"
  href="https://mallary.ai"
  horizontal
>
  <ol>
    <li>Sign up at https://mallary.ai</li>
    <li>Connect your social media accounts in the Mallary dashboard</li>
    <li>Approve the access requested by your app, or get a Mallary API key for a manual setup</li>
  </ol>
</Card>

## OAuth

OAuth-ready clients discover Mallary's authorization server automatically from:

```text
https://mallary.ai/.well-known/oauth-protected-resource
```

Mallary uses the authorization code flow with PKCE. The authorization server is:

```text
https://auth.mallary.ai
```

Mallary asks the signed-in user to approve the exact access the app needs. Mallary never gives the app the user's password or connected social account tokens.

ChatGPT and Codex can register a dedicated OAuth client through Mallary's dynamic client registration endpoint. Access tokens are short lived. Refresh tokens rotate, and revoked grants stop working.

Users can review and revoke OAuth access at `https://mallary.ai/oauth-connections`.

## OAuth Scopes

- `mallary.read`: read posts, comments, analytics, profiles, platforms, settings, and webhooks
- `mallary.publish`: upload media and create or schedule posts
- `mallary.engage`: reply to comments
- `mallary.manage`: change settings, profiles, webhooks, connections, and pending posts

## API Key Header

For a manual API-key setup, send the key as a bearer token:

```http
Authorization: Bearer <mallary_api_key>
```

## Identity Resolution

Identity is always resolved from the OAuth grant or API key owner. You do not separately pass a user ID when calling MCP tools.

## Base URL

Mallary MCP is served from:

```text
https://mallary.ai
```

## Endpoint

Use the MCP server at:

```text
/mcp
```
