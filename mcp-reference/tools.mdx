...
The thumbnail URL must already point to `https://files.mallary.ai/...`. YouTube regular videos and Shorts, Facebook feed videos, and Instagram videos/Reels can use custom thumbnails or covers. TikTok video posts use a frame from the video instead of a separate image. TikTok photo posts can use `thumbnail_url` only when it matches one of the supplied photo URLs, and that image becomes the photo cover.
