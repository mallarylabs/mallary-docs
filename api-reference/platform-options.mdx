...
```json
{
  "platform_options": {
    "instagram": {
      ...
      "brandedContentSponsors": ["@brandone", "17841400000000000"]
    }
  }
}
```
...
```json
{
  "platform_options": {
    "youtube": {
      "tags": ["social media automation", "creator tips", "Mallary"]
    }
  }
}
```
...
Add `thumbnail_url` to a YouTube video in `media`, or use `platform_options.youtube.thumbnail_url` for a YouTube-only cover. Use a JPG or PNG file stored by Mallary. The image can be up to 50 MB.
