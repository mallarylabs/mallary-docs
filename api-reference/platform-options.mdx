...
Set `platform_options.tiktok.post_mode` to `DIRECT_POST` to publish from Mallary. Use `MEDIA_UPLOAD` to send the post to your TikTok inbox so you can finish it in the TikTok app.
...
Snapchat supports Stories, Saved Stories, and Spotlight posts.

### Snapchat post type

Set `platform_options.snapchat.contentType` to `story`, `saved_story`, or `spotlight`.

### Snapchat title or description

Use `platform_options.snapchat.message` for the text that goes with a Saved Story or Spotlight post.

### Snapchat image or video

Add one image or one MP4 video to `media`. Stories and Saved Stories can use an image or video. Spotlight uses a video.
