...
Add `thumbnail_url` to a YouTube video in `media`, or use `platform_options.youtube.thumbnail_url` for a YouTube-only cover. Use a JPG or PNG file stored by Mallary. Use 16:9 for a regular video and 9:16 for a Short. The image can be up to 2 MB.
